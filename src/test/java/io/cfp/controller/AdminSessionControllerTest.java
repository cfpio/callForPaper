...
@WebMvcTest(io.cfp.controller.AdminSessionController.class)
@Deprecated
public class AdminSessionControllerTest {
    ...
    public void should_export_proposals_to_sched_format() throws Exception {
        ...
        List<EventSched> scheds = new ArrayList<>();
        EventSched eventSched = new EventSched();
        eventSched.setId("ID");
        scheds.add(eventSched);


        when(talkAdminService.exportSched(anyVararg())).thenReturn(scheds);

        mockMvc.perform(get("/v0/admin/sessions/export/sched.json")
    ...
    public void should_export_proposals_to_csv_format() throws Exception {
        ...
        mockMvc.perform(get("/v0/admin/sessions/export/sessions.csv")
            .accept("text/csv")
            .contentType(MediaType.APPLICATION_JSON_UTF8)
            .header("Authorization", "Bearer " + token)
        )
