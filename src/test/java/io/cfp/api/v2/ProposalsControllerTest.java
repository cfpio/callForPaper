...
import io.cfp.api.v2.ProposalsV2Controller;
...
@WebMvcTest(ProposalsV2Controller.class)
public class ProposalsControllerTest {
    ...
    public void should_get_proposals() throws Exception {
        ...
        when(proposalMapper.count(any(ProposalQuery.class))).thenReturn(proposals.size());
        ...
        mockMvc.perform(get("/api/proposals?page=1&size=2")
            .accept(CFPMediaType.APPLICATION_VND_CFP_IO_V2)
            .contentType(CFPMediaType.APPLICATION_VND_CFP_IO_V2)
            .header("Authorization", "Bearer "+token)
        )
            .andDo(print())
            .andExpect(status().isOk())
            .andExpect(content().contentType(CFPMediaType.APPLICATION_VND_CFP_IO_V2_VALUE))
            .andExpect(jsonPath("$.proposals[0].id").value("10"))
            .andExpect(jsonPath("$.totalElements").value(proposals.size()))
